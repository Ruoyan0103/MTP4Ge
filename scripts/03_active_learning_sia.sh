...
#
# Prerequisites:
#   python scripts/generate_sia_lammps.py   # creates data/sia/*.lammps
#
# Usage:
#   bash scripts/03_active_learning_sia.sh [--iter-per-structure N]
...
if [[ ! -f data/sia/tet.lammps ]]; then
    ...
    python scripts/generate_sia_lammps.py
fi
...
bash scripts/02_test_errors.sh "$AL_POT" errors "$TRAIN_CFG"
