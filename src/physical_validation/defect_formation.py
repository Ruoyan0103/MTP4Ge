...
Backends (--backend):
  mlp     - `mlp relax` / `mlp calculate_efs` (default).
  lammps  - LAMMPS (pair_style hybrid/overlay mtp nlh, see
            config/lammps/relax.in and src/physical_validation/energy_volume.py's LAMMPS
            single-point evaluator) for both relaxation stages instead.
            Needed for potentials trained with a radial basis type that
            `mlp relax` in the mlip-3-prune build cannot load (fails with
            "Wrong radial basis type") — see
...
Usage:
    python src/physical_validation/defect_formation.py --pot results/potentials/pot_al.almtp
    python src/physical_validation/defect_formation.py --pot results/potentials/pot_al.almtp \\
        --a0 5.779 --fmax 0.0001 --max-steps 500
    python src/physical_validation/defect_formation.py --pot results/potentials/20.mtp --backend lammps \\
...
DFT_REF = _load_ref_dict("defect_formation_energy/defect_formation_energy_DFT.dat")
GAP_REF = _load_ref_dict("defect_formation_energy/defect_formation_energy_GAP.dat")
...
def _cohesive_mlp(mlp: str, pot: str, mu: float, workdir: Path,
...
# ---------------------------------------------------------------------------
# Relaxation and cohesive energy via LAMMPS (pair_style hybrid/overlay mtp nlh)
# ---------------------------------------------------------------------------

def _write_lammps_data(cell: np.ndarray, pos: np.ndarray, path: Path) -> None:
...
def _relax_lammps(
    ...
    """
    Relax a configuration with LAMMPS (pair_style hybrid/overlay mtp nlh).

    fix_cell=True  -> ISIF=2 analog: only ionic positions relax (min_style cg,
                       fixed box).
    fix_cell=False -> ISIF=3 analog: `fix box/relax aniso 0.0` + minimize, so
                       ions AND the (diagonal) cell relax together.

    Returns (final energy [eV], relaxed cell [3x3 Å], relaxed positions [Nx3 Å]).
    `fmax` is only used for the post-hoc convergence check/status message —
    LAMMPS is driven by `max_steps` (maxiter/maxeval) with a tight internal ftol.
    """
    workdir.mkdir(parents=True, exist_ok=True)
    n_atoms = len(pos)

    _write_lammps_data(cell, pos, workdir / "structure.lammps")

    box_relax_line = "" if fix_cell else "fix cellrelax all box/relax aniso 0.0"
    unfix_line = "" if fix_cell else "unfix cellrelax"
    ...
    text = (
        ...
        .replace("${FTOL}", "1.0e-10")
        .replace("${MAXITER}", str(max_steps))
        .replace("${MAXEVAL}", str(max_steps * 10))
        .replace("${BOX_RELAX_LINE}", box_relax_line)
        .replace("${UNFIX_LINE}", unfix_line)
    )
    (workdir / "in.relax").write_text(text)

    cmd = shlex.split(lammps_cmd) + [
        "-in", "in.relax", "-log", "log.lammps", "-screen", "none",
    ]
    print("  LAMMPS:", " ".join(cmd), f"(cwd={workdir})")
    ...
    energy = float(energy_file.read_text().split()[0])

    forces = _parse_forces_from_dump(workdir / "force.dump", n_atoms)
    max_f = float(np.linalg.norm(forces, axis=1).max())

    from ase.io import read as ase_read
    relaxed = ase_read(str(workdir / "relaxed.data"), format="lammps-data", atom_style="atomic")
    rlx_cell = relaxed.get_cell().array.copy()
    rlx_pos = relaxed.get_positions()
...
def _cohesive_lammps(lammps_cmd: str, pot: str, mu: float, workdir: Path,
                      box_size: float = 25.0) -> float:
    """Cohesive energy per atom: E_coh = E_atom(isolated) − μ_Ge."""
    workdir.mkdir(parents=True, exist_ok=True)
    cell, pos, _types = build_isolated_atom(box_size)
    e_atom = _run_lammps_energy(lammps_cmd, pot, cell, pos, workdir)
    e_coh = e_atom - mu
    print(f"    E_atom(isolated) = {e_atom:.6f} eV | μ_Ge = {mu:.6f} eV/atom")
    print(f"    E_cohesive = {e_coh:.6f} eV/atom")
    return e_coh
...
def run(pot: str, outdir: Path, a0: float,
    ...
    relax_fn = _relax_mlp if backend == "mlp" else _relax_lammps
    cohesive_fn = _cohesive_mlp if backend == "mlp" else _cohesive_lammps
    engine = mlp if backend == "mlp" else lammps_cmd

    def _single_point(cell, pos, types, out_dir, tag):
        if backend == "mlp":
            write_bare_cfg([(cell, pos, types)], out_dir / f"{tag}.cfg")
            calc_efs(mlp, pot, out_dir / f"{tag}.cfg", out_dir / f"{tag}_efs.cfg")
            e, _ = _parse_efs(out_dir / f"{tag}_efs.cfg")
            return e
        return _run_lammps_energy(lammps_cmd, pot, cell, pos, out_dir)
    ...
    if no_relax:
        e_bulk = _single_point(b_cell, b_pos, b_types, b_out, "bulk")
    else:
        e_bulk, b_cell, b_pos = relax_fn(engine, pot, b_cell, b_pos, b_types, b_out,
    ...
    # ── Cohesive energy ──
    print("\n  [Cohesive energy]")
    e_coh = cohesive_fn(engine, pot, mu, outdir / "cohesive")
    ...
    for name, (builder, _) in DEFECTS.items():
        ...
        try:
            if no_relax:
                e_def = _single_point(d_cell, d_pos, d_types, d_out, "defect")
            else:
                e_def, _, _ = relax_fn(engine, pot, d_cell, d_pos, d_types, d_out,
    ...
    engine_tag = "" if backend == "mlp" else " (via LAMMPS)"
    relax_note = "no relaxation" if no_relax else (
        f"bulk: {'BFGS' if backend == 'mlp' else 'LAMMPS cg box/relax aniso'} "
        f"cell+ions (ISIF=3 analog); "
        f"defects: {'BFGS' if backend == 'mlp' else 'LAMMPS cg'} "
        f"fixed-cell ions-only (ISIF=2 analog), fmax={fmax} eV/Å"
    )
    with open(out_txt, "w") as f:
        f.write(f"Point defect formation energies — MTP: {pot}{engine_tag}\n")
...
def main() -> None:
    ...
    parser.add_argument("--config",     default=DEFAULT_TRAIN_CONFIG,
                        help="Training YAML (provides mlp_binary; --backend mlp only)")
