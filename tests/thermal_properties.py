...
Force constants are evaluated at N volumes via mlp calculate_efs; PhonopyQHA
fits the free-energy surface to extract thermodynamic properties vs temperature.

Usage:
    python tests/thermal_properties.py --pot results/potentials/pot_al.almtp
    python tests/thermal_properties.py --pot results/potentials/pot_al.almtp \\
...
def _plot_thermal(
    ...
    ax = axes[0]
    dft_te = _load_ref(ref_dir / 'thermal_expansion_coefficient_dft.dat')
    ...
    exp_te = _load_ref(ref_dir / 'thermal_expansion_experimental_Reeber1996.dat', skiprows=1)
    ...
    ax = axes[1]
    dft_cp = _load_ref(ref_dir / 'heat_capacity_vs_temperature_dft.dat')
    if dft_cp is not None:
        ax.plot(dft_cp[:, 0], dft_cp[:, 1] / 2.0,
                label='DFT', color='black', linewidth=1)
    ax.plot(cp_temps, cp_vals, label='MTP', color='red', linewidth=1)
    cp_exp_cfg = [
        ('heat_capacity_experimental_Estermann1952.dat', 'Exp. Estermann et al.', 'blue',   '^'),
        ('heat_capacity_experimental_Flubacher1953.dat', 'Exp. Flubacher et al.', 'green',  's'),
        ('heat_capacity_experimental_Leadbetter1969.dat', 'Exp. Leadbetter et al.', 'orange', 'o'),
    ]
    for fname, lbl, col, mrk in cp_exp_cfg:
        d = _load_ref(ref_dir / fname, skiprows=1)
        if d is not None:
    ...
    # ---- (e) Bulk modulus ----
    ax = axes[2]
    dft_bm = _load_ref(ref_dir / 'bulk_modulus_vs_temperature_dft.dat')
    if dft_bm is not None:
        ax.plot(dft_bm[:, 0], dft_bm[:, 1],
                label='DFT', color='black', linewidth=1)
    ax.plot(bm_temps, bm_vals, label='MTP', color='red', linewidth=1)
    bm_exp = _load_ref(ref_dir / 'bulk_modulus_experimental_Yin1982.dat', skiprows=1)
