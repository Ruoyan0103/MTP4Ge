...
Computes 1st- and 2nd-nearest-neighbour vacancy hop barriers using
ASE NEB with MTPCalculator (mlp calculate_efs per image).
...
def run_neb(
    ...
    for i in range(n_images):
        img = initial.copy()
        img.calc = MTPCalculator(mlp=mlp, pot=pot)
        images.append(img)
    images.append(final.copy())

    # Set calculators on endpoints too (required for energy queries)
    images[0].calc = MTPCalculator(mlp=mlp, pot=pot)
    images[-1].calc = MTPCalculator(mlp=mlp, pot=pot)
...
def run(pot: str, outdir: Path, mlp: str, a0: float,
    ...
    try:
        ...
        fig.suptitle(f"Ge Vacancy Migration — MTP", fontsize=12)
        fig.tight_layout()
