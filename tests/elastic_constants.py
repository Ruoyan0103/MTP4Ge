...
def _load_elastic_ref() -> dict[str, float] | None:
    """Load experimental elastic constants from reference file."""
    path = REF_DIR / "elastic_constants_experimental.dat"
    if not path.exists():
        return None
    with open(path) as f:
        for line in f:
...
EXP_ELASTIC = _load_elastic_ref() or {"C11": 126, "C12": 48, "C44": 67, "B": 74}
...
def run(
    ...
    with open(out_txt, "w") as f:
        ...
        f.write(f"C12  = {res['C12']:8.2f} GPa    (exp. Ge:  ~48 GPa) (DFT. Ge: ~37 GPa)\n")
        f.write(f"C44  = {res['C44']:8.2f} GPa    (exp. Ge:  ~67 GPa) (DFT. Ge: ~54 GPa)\n\n")
        ...
        f.write(f"Bulk modulus     B  = {res['B']:.2f} GPa (exp. Ge: ~77 GPa) (DFT. Ge: ~59 GPa) \n")
